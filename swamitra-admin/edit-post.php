...
if (strlen($_SESSION['login']) == 0) { 
    ...
    if (isset($_POST['update'])) {
        ...
        $catid = intval($_POST['category']); // Convert to integer to prevent SQL injection
        $postdetails = mysqli_real_escape_string($con, $_POST['postdescription']);
        $lastuptdby = $_SESSION['login'];
        $arr = explode(" ", $posttitle);
        $url = implode("-", $arr);
        $status = 1;
        $postid = intval($_GET['pid']); // Convert to integer to prevent SQL injection

        // Check if a new image is uploaded
        if ($_FILES["postimage"]["name"] != '') {
            // Get the name of the new uploaded image
            $image_name = mysqli_real_escape_string($con, $_FILES["postimage"]["name"]);
            ...
            $query = "UPDATE tblposts SET PostTitle='$posttitle', CategoryId='$catid', PostDetails='$postdetails', PostUrl='$url', Is_Active='$status', PostImage='$image_name', lastUpdatedBy='$lastuptdby' WHERE id = $postid";
        } else {
            // If no new image is uploaded, update other fields without changing the image
            $query = "UPDATE tblposts SET PostTitle='$posttitle', CategoryId='$catid', PostDetails='$postdetails', PostUrl='$url', Is_Active='$status', lastUpdatedBy='$lastuptdby' WHERE id = $postid";
        }
...
$query=mysqli_query($con,"select tblposts.id as postid,tblposts.PostImage as postimages ,tblposts.PostTitle as title,tblposts.PostDetails,tblcategory.CategoryName as category,tblcategory.id as catid,tblsubcategory.SubCategoryId as subcatid,tblsubcategory.Subcategory as subcategory from tblposts left join tblcategory on tblcategory.id=tblposts.CategoryId left join tblsubcategory on tblsubcategory.SubCategoryId=tblposts.SubCategoryId where tblposts.id='$postid' and tblposts.Is_Active=1 ");
...
<!--</div>-->
       ...
       <div class="row">
                                        <div class="col-sm-12">
                                            <div class="card-box">
                                                <h4 class="m-b-30 m-t-0 header-title"><b>Post Image</b></h4>
                                                <td><img src="postimages/<?php echo htmlentities($row['postimages']);?>" alt="Image" width="100" height="100"></td>
                                                <br />
